...
class rect:
    def __init__(self):
        ...
        self.rdoverlay = False if self.luoverlay else True if randint(
            1, 10) > 8 else False
...
class captchatext:
    ...
    def draw(self, image):
        ...
        font = ImageFont.truetype(
            FONTPATH[randint(0, 1)], randint(25, 27) * 10)
        text = Image.new("RGBA", (font.getsize(
            self.letter)[0], 300), (0, 0, 0, 0))
        ...
        rand_min = (self.offset - base - 4) if (self.offset -
                                                base - 4) >= -15 else -15
        rand_min = 0 if self.priority == 0 else rand_min
        avg_dp = int(200 / self.captchalen)
        rand_max = (
            avg_dp - text.size[0]) if self.priority == self.captchalen - 1 else (avg_dp - text.size[0] + 10)
        try:
...
def generate(GENNUM, SAVEPATH, ENGP=25, FIVEP=0, ENGNOLIMIT=False):
    captchacsv = open(SAVEPATH + "captcha_train.csv",
                      'w', encoding='utf8', newline='')
    numcsv = open(SAVEPATH+"len_train.csv", 'w', encoding='utf8', newline='')
    letterlist = []
    numlist = []
    for index in range(1, GENNUM + 1, 1):
        print(index, end='\r')
        captchastr = ""
        captchalen = 5 if randint(1, 100) <= FIVEP else 5
        engat = randint(0, captchalen - 1) if randint(1, 100) <= ENGP else -1
        bgcolor = [randint(180, 250) for _ in range(3)]
        captcha = Image.new('RGBA', (200, 60),
        ...
        for i in range(captchalen):
            newtext = captchatext(i, offset, captchalen,
                                  (True if engat == i else False), ENGNOLIMIT)
        ...
        if index > 999:
            ...
            picname = '00'+str(index)
        else:
            picname = '000'+str(index)
        letterlist.append([picname, captchastr])
        numlist.append([picname, '5'])
        for obj in rectlist:
            obj.draw(image=captcha, overlay=True)
        

        captcha.convert("RGB").save(SAVEPATH + picname + ".jpg", "JPEG")

    writer = csv.writer(captchacsv)
    writer.writerows(letterlist)
    captchacsv.close()
    writer = csv.writer(numcsv)
    writer.writerows(numlist)
    numcsv.close()


if __name__ == "__main__":
    generate(9999, "./data/5_imitate_train_set/",  ENGP=100, FIVEP=50, ENGNOLIMIT=True)
